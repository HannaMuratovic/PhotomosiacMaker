...
int main(){
   ...
   for (int i = 0; i < file_names.size(); i++){
     ...
     CImg<int> tile_image(file_names[i].c_str());
     auto tile_avg = tile_data.average;
     rgbAverage(tile_image, tile_avg);
     tile_data.average = tile_avg;
     store_tiles[i] = tile_data;

     if (tile_image.height() > max_height){
       max_height = tile_image.height();
     }
     if (tile_image.width() > max_width){
       max_width = tile_image.width();
     }
    }

    //Mat main_image = imread(main_image_path, 1);
    CImg<int> main_image(main_image_path.c_str());
